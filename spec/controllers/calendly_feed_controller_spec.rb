...
RSpec.describe CalendlyFeedController, type: :controller do
  ...
  describe 'GET index' do
    ...
    it 'has a canceled event' do
      ...
      get :index
      expect(response.body).to include('<span class="canceled-event">Canceled: </span>')
      expect(response.body).to include('test@example.com')
      expect(response.body).to include('April 09, 2019 at  2:30pm')
      expect(response.body).to include('for 15 minutes')
    end
  ...
  describe 'POST webhook_catch' do
    ...
    it 'catches wrong event creation' do
    ...
    it 'catches cancelation' do
      ...
      expect(response.content_type).to eq 'application/json'
      expect(response.body).to eq('{"success":true}')
      event = Event.find_by(uuid: 'BBBBBBBBBBBBBBBB')
      expect(event.canceled).to eq(true)
    end

    it 'catches wrong event cancelation' do
      post :webhook_catch, params: JSON.parse('{"event": "invitee.canceled","everything_else_is": "wrong"}'), format: :json
      expect(response.content_type).to eq('application/json')
      expect(response.body).to eq('{"success":false}')
    end
  end
